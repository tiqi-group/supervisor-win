            ...
            if not processname:
                ...
                try:
                    ...
                    if e.code == Faults.NO_FILE:
                        ...
                        tail = "ERROR: unexpected rpc fault [%d] %s" % (e.code, e.text)
    ...
    def make_callback(self, namespec, action):
        ...
        if action:
            if action == "refresh":
                ...
                if action == "start":
                    try:
                        bool_or_callback = rpcinterface.supervisor.startProcess(
                            namespec
                        )
                    except RPCError as e:
                        if e.code == Faults.NO_FILE:
                    ...
                    try:
                        bool_or_callback = rpcinterface.supervisor.stopProcess(namespec)
                    except RPCError as e:
                        msg = "unexpected rpc fault [%d] %s" % (e.code, e.text)
    ...
    def render(self):
        ...
        if data:
            ...
            for tr_element, item in iterator:
                ...
                anchor.attributes(
                    href="tail.html?processname=%s" % urllib.quote(processname)
                )
                ...
                for li_element, actionitem in actionitem_td.repeat(actions):
                    ...
                    if actionitem is None:
                        ...
                        anchor.attributes(
                            href=actionitem["href"], name=actionitem["name"]
                        )
...
VIEWS = {
    "index.html": {"template": "ui/status.html", "view": StatusView},
    "tail.html": {
