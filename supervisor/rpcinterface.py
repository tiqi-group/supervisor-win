...
from supervisor.states import RUNNING_STATES, STOPPED_STATES, SIGNALLABLE_STATES
...
class SupervisorNamespaceRPCInterface(object):
    ...
    def _update(self, text):
        ...
        if (
            isinstance(self.supervisord.options.mood, int)
            and self.supervisord.options.mood < SupervisorStates.RUNNING
        ):
...
def _total_seconds(timedelta):
    return (
        (timedelta.days * 86400 + timedelta.seconds) * 10**6 + timedelta.microseconds
    ) / 10**6
...
def make_allfunc(processes, predicate, func, **extra_kwargs):
    ...
    def allfunc(
        ...
        results=results,
    ):  # used only to fool scoping, never passed by caller
