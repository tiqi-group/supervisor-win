...
class ServerOptions(Options):
    ...
    def __init__(self):
        ...
        self.add("delaysecs", "supervisord.delaysecs", "p", "delaysecs", default=0.5)
    ...
    def realize(self, *arg, **kw):
        ...
        if self.serverurl is None:
            for config in [
                ...
                if config["family"] is socket.AF_INET
            ]:
                host = config["host"]
                port = config["port"]
                if not host:
    ...
    def process_groups_from_parser(self, parser):
        ...
        for section in all_sections:
            ...
            for program in programs:
                ...
                if (
                    not program_section in all_sections
                    and not fcgi_section in all_sections
                ):
                    raise ValueError(
                        "[%s] names unknown program or fcgi-program %s"
                        % (section, program)
                    )
                ...
                processes = self.processes_from_section(
        ...
        for section in all_sections:
            ...
            if buffer_size < 1:
                raise ValueError(
                    "[%s] section sets invalid buffer_size (%d)"
                    % (section, buffer_size)
                )
            ...
            for pool_event_name in pool_event_names:
                ...
                if pool_event is None:
                    raise ValueError(
                        "Unknown event type %s in [%s] events"
        ...
        for section in all_sections:
            if (
                not section.startswith("fcgi-program:")
            ...
            groups.append(
                FastCGIGroupConfig(
                    self, program_name, priority, processes, socket_config
                )
            )
    ...
    def server_configs_from_parser(self, parser):
        ...
        if self._parse_servernames(parser, "unix_http_server"):
            raise ValueError(
                "section [unix_http_server] not supported.\n"
    ...
    def make_logger(self):
        ...
        if self.nodaemon and not self.silent:
            loggers.handle_stdout(self.logger, format, stdout=sys.stdout or self.stdout)
        loggers.handle_file(
            self.logger,
            self.logfile,
            format,
...
class ClientOptions(Options):
    ...
    def read_config(self, fp):
        ...
        # connection timeout
        section.xmlrpc_timeout = parser.getdefault(
            "xmlrpc_timeout", section.xmlrpc_timeout
        )
    ...
    def getServerProxy(self):
        return xmlrpclib.ServerProxy(
            ...
            transport=xmlrpc.SupervisorTransport(
                self.username, self.password, self.serverurl, self.xmlrpc_timeout
            ),
        )
