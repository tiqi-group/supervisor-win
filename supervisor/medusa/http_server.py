...
class http_request(object):
    ...
    def remove_header(self, name, value=None):
        ...
        if name in self.reply_headers and (
            value is None or self.reply_headers[name] == value
        ):
    ...
    def collect_incoming_data(self, data):
        if self.collector:
            ...
            self.log_info(
                "Dropping %d bytes of incoming request data" % len(data), "warning"
            )
...
class http_server(asyncore.dispatcher):
    ...
    def __init__(self, ip, port, resolver=None, logger_object=None, init=False):
        self.ip = ip
        self.port = port
        asyncore.dispatcher.__init__(self)
        if init:
            self._pos_init(ip, port, resolver, logger_object)

    def _pos_init(self, ip, port, resolver, logger_object):
