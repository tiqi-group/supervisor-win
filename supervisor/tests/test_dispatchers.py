...
class POutputDispatcherTests(unittest.TestCase):
    ...
    def _makeOne(self, process, channel="stdout"):
        ...
        events = {
            "stdout": events.ProcessCommunicationStdoutEvent,
    ...
    def test_handle_read_event(self):
        ...
        config = DummyPConfig(
            options, "process1", "/bin/process1", stdout_capture_maxbytes=100
        )
        process = DummyProcess(config)
        dispatcher = self._makeOne(process)
    ...
    def test_handle_error(self):
        ...
        self.assertTrue(
            result.startswith("uncaptured python exception, closing channel"), result
        )
    ...
    def test_record_output_emits_stdout_event_when_enabled(self):
        options = DummyOptions()
        config = DummyPConfig(
            options, "process1", "/bin/process1", stdout_events_enabled=True
        )
        process = DummyProcess(config)
    ...
    def test_record_output_does_not_emit_stdout_event_when_disabled(self):
        options = DummyOptions()
        config = DummyPConfig(
            options, "process1", "/bin/process1", stdout_events_enabled=False
        )
        process = DummyProcess(config)
        dispatcher = self._makeOne(process, "stdout")
        dispatcher.output_buffer = b"hello from stdout"
    ...
    def test_record_output_emits_stderr_event_when_enabled(self):
        options = DummyOptions()
        config = DummyPConfig(
            options, "process1", "/bin/process1", stderr_events_enabled=True
        )
        process = DummyProcess(config)
    ...
    def test_record_output_does_not_emit_stderr_event_when_disabled(self):
        options = DummyOptions()
        config = DummyPConfig(
            options, "process1", "/bin/process1", stderr_events_enabled=False
        )
        process = DummyProcess(config)
        dispatcher = self._makeOne(process, "stderr")
        dispatcher.output_buffer = b"hello from stderr"
    ...
    def test_record_output_capturemode_string_longer_than_token(self):
        ...
        dispatcher.record_output()
        self.assertEqual(
            dispatcher.childlog.data, [b"stdout string longer than a token"]
        )
        self.assertEqual(
    ...
    def test_ctor_capturelog_only(self):
        options = DummyOptions()
        config = DummyPConfig(
            options, "process1", "/bin/process1", stdout_capture_maxbytes=300
        )
        process = DummyProcess(config)
        dispatcher = self._makeOne(process)
        self.assertEqual(dispatcher.process, process)
    ...
    def test_ctor_stdout_logfile_is_empty_string(self):
        ...
        options = DummyOptions()
        config = DummyPConfig(
            options, "process1", "/bin/process1", stdout_logfile=logfile_name("")
        )
        process = DummyProcess(config)
        dispatcher = self._makeOne(process)
        self.assertEqual(dispatcher.process, process)
    ...
    def test_ctor_stdout_logfile_none_and_stdout_syslog_true(self):
        ...
        self.assertEqual(dispatcher.normallog.handlers[0].__class__, SyslogHandler)
    ...
    def test_ctor_stdout_logfile_str_and_stdout_syslog_true(self):
        ...
        self.assertTrue(
            any(isinstance(h, FileHandler) for h in dispatcher.normallog.handlers)
        )
        self.assertTrue(
            any(isinstance(h, SyslogHandler) for h in dispatcher.normallog.handlers)
        )
        dispatcher.normallog.close()
...
class PInputDispatcherTests(unittest.TestCase):
    ...
    def test_handle_write_event_epipe_raised(self):
        ...
        options.write_exception = OSError(errno.EPIPE, os.strerror(errno.EPIPE))
        dispatcher.handle_write_event()
        self.assertEqual(dispatcher.input_buffer, b"")
        self.assertTrue(
            options.logger.data[0].startswith("fd 0 closed, stopped monitoring")
        )
    ...
    def test_handle_write_event_uncaught_raised(self):
        ...
        options.write_exception = OSError(errno.EBADF, os.strerror(errno.EBADF))
...
class PEventListenerDispatcherTests(unittest.TestCase):
    ...
    def test_handle_read_event_nodata(self):
        ...
        self.assertEqual(
            dispatcher.process.listener_state, EventListenerStates.ACKNOWLEDGED
        )
    ...
    def test_handle_read_event_logging_childlog(self):
        ...
        self.assertEqual(len(dispatcher.childlog.data), 1)
        self.assertEqual(
            dispatcher.childlog.data[0], b"supercalifragilisticexpialidocious"
        )
    ...
    def test_handle_listener_state_change_busy_to_unknown(self):
        ...
        self.assertEqual(dispatcher.state_buffer, b"")
        self.assertEqual(
            options.logger.data[0], "process1: bad result line: 'bogus data'"
        )
        self.assertEqual(options.logger.data[1], "process1: BUSY -> UNKNOWN")
        self.assertEqual(
            options.logger.data[2],
            "process1: has entered the UNKNOWN state and will "
            "no longer receive events, this usually indicates "
            "the process violated the eventlistener protocol",
        )
