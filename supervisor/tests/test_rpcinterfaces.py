...
class MainXMLRPCInterfaceTests(TestBase):
    ...
    def test_traverse(self):
        ...
        self._assertRPCError(
            xmlrpc.Faults.INCORRECT_PARAMETERS,
            xmlrpc.traverse,
            interface,
            "dummy.hello",
            [1],
...
class SupervisorNamespaceXMLRPCInterfaceTests(TestBase):
    ...
    def test_readLog_unreadable(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.NO_FILE, interface.readLog, offset=0, length=1
        )
    ...
    def test_readLog_badargs(self):
        ...
        try:
            ...
            with open(logfile, "w+") as f:
                f.write("x" * 2048)
            self._assertRPCError(
                xmlrpc.Faults.BAD_ARGUMENTS, interface.readLog, offset=-1, length=1
            )
            self._assertRPCError(
                xmlrpc.Faults.BAD_ARGUMENTS, interface.readLog, offset=-1, length=-1
            )
    ...
    def test_clearLog_unremoveable(self):
        ...
        options.remove_exception = OSError(errno.EPERM, os.strerror(errno.EPERM))
        supervisord = DummySupervisor(options)
        interface = self._makeOne(supervisord)
    ...
    def test_addProcessGroup(self):
        ...
        self._assertRPCError(
            xmlrpc.Faults.ALREADY_ADDED, interface.addProcessGroup, "group1"
        )
        self.assertEqual(list(supervisord.process_groups.keys()), ["group1"])
    ...
    def test_removeProcessGroup_bad_name(self):
        ...
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.removeProcessGroup, "asdf"
        )
    ...
    def test_removeProcessGroup_still_running(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.STILL_RUNNING, interface.removeProcessGroup, "group1"
        )
    ...
    def test_startProcess_already_started(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.ALREADY_STARTED, interface.startProcess, "foo"
        )
    ...
    def test_startProcess_unknown_state(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(xmlrpc.Faults.FAILED, interface.startProcess, "foo")
    ...
    def test_startProcess_bad_group_name(self):
        ...
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.startProcess, "group2:foo"
        )
    ...
    def test_startProcess_bad_process_name(self):
        ...
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.startProcess, "group1:bar"
        )
    ...
    def test_startProcess_bad_command(self):
        ...
        self._assertRPCError(
            xmlrpc.Faults.NOT_EXECUTABLE, interface.startProcess, "foo"
        )
    ...
    def test_startProcess(self):
        ...
        pconfig = DummyPConfig(
            options, "foo", __file__, autostart=False, startsecs=0.01
        )
        from supervisor.process import ProcessStates
    ...
    def test_startProcess_splat_calls_startProcessGroup(self):
        options = DummyOptions()
        pconfig1 = DummyPConfig(
            options, "process1", __file__, autostart=False, startsecs=0.01
        )
        pconfig2 = DummyPConfig(
            options, "process2", __file__, priority=2, startsecs=0.01
        )
    ...
    def test_startProcessGroup(self):
        options = DummyOptions()
        pconfig1 = DummyPConfig(
            options, "process1", __file__, priority=1, startsecs=0.01
        )
        pconfig2 = DummyPConfig(
            options, "process2", __file__, priority=2, startsecs=0.01
        )
    ...
    def test_stopProcess_success_in_onwait(self):
        ...
        supervisord = PopulatedDummySupervisor(options, "foo", pconfig)
        process = supervisord.process_groups["foo"].processes["foo"]
        L = [ProcessStates.RUNNING, ProcessStates.STOPPING, ProcessStates.STOPPED]
    ...
    def test_stopProcess_NDY_in_onwait(self):
        ...
        supervisord = PopulatedDummySupervisor(options, "foo", pconfig)
        process = supervisord.process_groups["foo"].processes["foo"]
        L = [ProcessStates.RUNNING, ProcessStates.STOPPING, ProcessStates.STOPPING]
    ...
    def test_signalProcessGroup_with_signal_number(self):
        ...
        interface = self._makeOne(supervisord)
        sig = signal.SIGABRT
        result = interface.signalProcessGroup("foo", sig)
        self.assertEqual(interface.update_text, "signalProcessGroup")
        # Sort so we get deterministic results despite hash randomization
        result = sorted(result, key=operator.itemgetter("name"))
    ...
    def test_signalProcessGroup_nosuchgroup(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.signalProcessGroup, "bar", 10
        )
    ...
    def test_getProcessInfo(self):
        ...
        options = DummyOptions()
        config = DummyPConfig(
            options, "foo", "/bin/foo", stdout_logfile="/tmp/fleeb.bar"
        )
    ...
    def test_getProcessInfo_bad_name_when_bad_process(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.getProcessInfo, "nonexistent"
        )
    ...
    def test_tailProcessStdoutLog_bad_name(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.tailProcessStdoutLog, "BAD_NAME", 0, 10
        )
    ...
    def test_tailProcessStdoutLog_bad_name_no_process(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.tailProcessStdoutLog, "foo:*", 0, 10
        )
    ...
    def test_tailProcessStdoutLog_all(self):
        ...
        pconfig = DummyPConfig(
            ...
            "foo",
            "/bin/foo",
            stdout_logfile=TempFileOpen.make_path("stdout-logfile-4.log"),
        )
        supervisord = PopulatedDummySupervisor(options, "foo", pconfig)
        interface = self._makeOne(supervisord)
        process = supervisord.process_groups["foo"].processes["foo"]
        ...
        try:
            ...
            data, offset, overflow = interface.tailProcessStdoutLog(
                "foo", offset=0, length=len(letters)
            )
    ...
    def test_tailProcessStdoutLog_none(self):
        ...
        pconfig = DummyPConfig(
            ...
            "foo",
            "/bin/foo",
            stdout_logfile=TempFileOpen.make_path("stdout-logfile-6.log"),
        )
        supervisord = PopulatedDummySupervisor(options, "foo", pconfig)
        interface = self._makeOne(supervisord)
        process = supervisord.process_groups["foo"].processes["foo"]
        ...
        try:
            ...
            # offset==logsize
            data, offset, overflow = interface.tailProcessStdoutLog(
                "foo", offset=len(letters), length=100
            )
            self.assertEqual(interface.update_text, "tailProcessStdoutLog")
            self.assertEqual(overflow, False)
            self.assertEqual(offset, len(letters))
            ...
            # offset > logsize
            data, offset, overflow = interface.tailProcessStdoutLog(
                "foo", offset=len(letters) + 5, length=100
            )
            self.assertEqual(interface.update_text, "tailProcessStdoutLog")
            self.assertEqual(overflow, False)
            self.assertEqual(offset, len(letters))
    ...
    def test_tailProcessStdoutLog_overflow(self):
        ...
        pconfig = DummyPConfig(
            ...
            "foo",
            "/bin/foo",
            stdout_logfile=TempFileOpen.make_path("stdout-logfile-7.log"),
        )
        supervisord = PopulatedDummySupervisor(options, "foo", pconfig)
        interface = self._makeOne(supervisord)
        process = supervisord.process_groups["foo"].processes["foo"]
    ...
    def test_tailProcessStdoutLog_unreadable(self):
        ...
        options = DummyOptions()
        pconfig = DummyPConfig(
            options, "foo", "/bin/foo", stdout_logfile="/tmp/fooooooo"
        )
        supervisord = PopulatedDummySupervisor(options, "foo", pconfig)
        interface = self._makeOne(supervisord)

        data, offset, overflow = interface.tailProcessStdoutLog(
            "foo", offset=0, length=100
        )
        self.assertEqual(interface.update_text, "tailProcessStdoutLog")
        self.assertEqual(overflow, False)
    ...
    def test_tailProcessStderrLog_bad_name(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.tailProcessStderrLog, "BAD_NAME", 0, 10
        )
    ...
    def test_tailProcessStderrLog_bad_name_no_process(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.tailProcessStderrLog, "foo:*", 0, 10
        )
    ...
    def test_tailProcessStderrLog_all(self):
        ...
        pconfig = DummyPConfig(
            ...
            "foo",
            "/bin/foo",
            stderr_logfile=TempFileOpen.make_path("stderr-logfile.log"),
        )
        supervisord = PopulatedDummySupervisor(options, "foo", pconfig)
        interface = self._makeOne(supervisord)
        process = supervisord.process_groups["foo"].processes["foo"]
        ...
        try:
            ...
            data, offset, overflow = interface.tailProcessStderrLog(
                "foo", offset=0, length=len(letters)
            )
    ...
    def test_tailProcessStderrLog_overflow(self):
        ...
        pconfig = DummyPConfig(
            ...
            "foo",
            "/bin/foo",
            stderr_logfile=TempFileOpen.make_path("stderr-logfile-2.log"),
        )
        supervisord = PopulatedDummySupervisor(options, "foo", pconfig)
        interface = self._makeOne(supervisord)
        process = supervisord.process_groups["foo"].processes["foo"]
    ...
    def test_tailProcessStderrLog_unreadable(self):
        ...
        options = DummyOptions()
        pconfig = DummyPConfig(
            options, "foo", "/bin/foo", stderr_logfile="/tmp/fooooooo"
        )
        supervisord = PopulatedDummySupervisor(options, "foo", pconfig)
        interface = self._makeOne(supervisord)
    ...
    def test_clearProcessLogs_bad_name_no_group(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.clearProcessLogs, "badgroup"
        )
    ...
    def test_clearProcessLogs_bad_name_no_process(self):
        ...
        interface = self._makeOne(supervisord)
        self._assertRPCError(
            xmlrpc.Faults.BAD_NAME, interface.clearProcessLogs, "foo:*"
        )
    ...
    def test_sendProcessStdin_reraises_other_oserrors(self):
        ...
        interface = self._makeOne(supervisord)
        self.assertRaises(
            OSError, interface.sendProcessStdin, "process1", "chars for stdin"
        )
...
class SystemNamespaceXMLRPCInterfaceTests(TestBase):
    ...
    def test_methodSignature(self):
        ...
        interface = self._makeOne()
        self._assertRPCError(
            xmlrpc.Faults.SIGNATURE_UNSUPPORTED, interface.methodSignature, ["foo.bar"]
        )
        ...
        for k in methods.keys():
            ...
            for thing in parsed:
                if thing[1] == "param":  # tag name
                    ...
                    raise AssertionError(
                        "unknown tag type %s for %s, parsed %s" % (thing[1], k, parsed)
                    )
            # param tokens
            ...
            if len(argnames) != len(pnames):
                raise AssertionError(
                    ...
                    "(%s args, %s doc params) in %s" % (len(argnames), len(pnames), k)
                )
            for docline in plines:
                self.assertTrue(
                    type(docline) == int, (docline, type(docline), k, parsed)
                )
            ...
            for docline in rlines:
                self.assertTrue(type(docline) == int, (docline, type(docline), k))
            for doctype in rtypes:
                self.assertTrue(doctype in _RPCTYPES, doctype)
            for docname in rnames:
                self.assertTrue(type(docname) == type(""), (docname, type(docname), k))
            for doctext in rtexts:
                self.assertTrue(type(doctext) == type(""), (doctext, type(doctext), k))
    ...
    def test_multicall_recursion_guard(self):
        ...
        e = xmlrpc.RPCError(
            xmlrpc.Faults.INCORRECT_PARAMETERS, "Recursive system.multicall forbidden"
        )
    ...
    def test_methodHelp(self):
        ...
        interface = self._makeOne()
        self._assertRPCError(
            xmlrpc.Faults.SIGNATURE_UNSUPPORTED, interface.methodHelp, ["foo.bar"]
        )
