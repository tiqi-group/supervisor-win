...
class IncludeTestsMixin(object):
    def test_read_config_include_with_no_files_raises_valueerror(self):
        ...
        try:
            ...
            self.assertEqual(
                exc.args[0], ".ini file has [include] section, but no files setting"
            )
    ...
    def test_read_config_include_with_no_matching_files_logs_warning(self):
        ...
        instance.read_config(StringIO(text))
        self.assertEqual(
            instance.parse_warnings, ['No file matches via include ".\\nonexistent\\*"']
        )
...
class ClientOptionsTests(unittest.TestCase, IncludeTestsMixin):
    ...
    def test_no_config_file(self):
        ...
        instance.realize(
            args=["-s", "http://localhost:9001", "-u", "chris", "-p", "123"]
        )
    ...
    def test_options_with_environment_expansions(self):
        ...
        self.assertEqual(
            options.history_file, "E:\\path\\to\\histdir\\.supervisorctl.hist"
        )
    ...
    def test_options_supervisorctl_section_expands_here(self):
        ...
        self.assertEqual(
            options.serverurl, "unix://" + os.path.join(here, "supervisord.sock")
        )
    ...
    def test_read_config_no_supervisord_section_raises_valueerror(self):
        ...
        try:
            ...
            self.assertEqual(
                exc.args[0], ".ini file does not include supervisorctl section"
            )
    ...
    def test_read_config_include_expands_here(self):
        conf = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "fixtures", "include.conf"
        )
...
class ServerOptionsTests(unittest.TestCase, IncludeTestsMixin):
    ...
    def test_options_ignores_space_prefixed_inline_comments(self):
        ...
        options = instance.configroot.supervisord
        self.assertEqual(
            options.logfile, os.path.join(os.environ["TMP"], "supervisord.log")
        )
    ...
    def test_read_config_logfile_with_nonexistent_dirpath(self):
        ...
        logfile_with_nonexistent_dir = os.path.join(
            os.path.dirname(__file__), "nonexistent", "supervisord.log"
        )
        text = lstrip(
            """\
        [supervisord]
    ...
    def test_read_config_no_supervisord_section_raises_valueerror(self):
        ...
        try:
            ...
            self.assertEqual(
                exc.args[0], ".ini file does not include supervisord section"
            )
    ...
    def test_check_execv_args_notexecutable(self):
        ...
        self.assertRaises(
            NotExecutable, instance.check_execv_args, windir, [windir], os.stat(windir)
        )
    ...
    def test_check_execv_args_isdir(self):
        ...
        self.assertRaises(
            NotExecutable, instance.check_execv_args, "/", ["/"], os.stat("/")
        )
    ...
    def test_realize_positional_args_not_supported(self):
        ...
        self.assertEqual(
            recorder[0], "positional arguments are not supported: %s" % args
        )
    ...
    def test_options_afinet_no_port(self):
        ...
        try:
            ...
            self.assertEqual(
                exc.args[0], "section [inet_http_server] has no port value"
            )
    ...
    def test_processes_from_section(self):
        ...
        self.assertEqual(
            pconfig.environment, {"KEY1": "val1", "KEY2": "val2", "KEY3": "0"}
        )
    ...
    def test_processes_from_section_process_num_expansion(self):
        ...
        for num in (0, 1):
            ...
            self.assertEqual(
                pconfigs[num].directory, os.path.join(tempdir, "foo_%d" % num)
            )
            self.assertEqual(
                pconfigs[num].stderr_logfile,
                os.path.join(tempdir, "foo_%d_stderr" % num),
            )
            self.assertEqual(
                pconfigs[num].stdout_logfile,
    ...
    def test_options_with_environment_expansions(self):
        ...
        self.assertEqual(
            instance.identifier, "supervisor_%(ENV_USER)s" % config.expansions
        )
        ...
        self.assertEqual(
            proc1.command,
            "/bin/customcat --logdir=%s" % os.path.join(tempdir, "path\\to\\logs"),
        )
    ...
    def test_options_expands_combined_expansions(self):
        ...
        text += (
            "command = %(here)s\\bin\\cat --foo=%(ENV_FOO)s "
        ...
        with open(supervisord_conf, "w") as f:
            f.write(text)
        try:
            instance.environ_expansions = {"ENV_LOGNAME": "mainlog", "ENV_FOO": "bar"}
            instance.configfile = supervisord_conf
            instance.realize(args=[])
        finally:
            shutil.rmtree(here, ignore_errors=True)
    ...
    def test_processes_from_section_bad_program_name_spaces(self):
        ...
        self.assertRaises(
            ValueError,
            instance.processes_from_section,
            config,
            "program:spaces are bad",
            None,
        )
    ...
    def test_processes_from_section_no_procnum_in_processname(self):
        ...
        config.read_string(text)
        self.assertRaises(
            ValueError, instance.processes_from_section, config, "program:foo", None
        )
    ...
    def test_processes_from_section_unexpected_end_of_key_value_pairs(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                "'KEY1=val1,KEY2=val2,KEY3' in section 'program:foo'" in str(e)
            )
        else:
    ...
    def test_event_listener_pool_disallows_buffer_size_zero(self):
        ...
        try:
            ...
            self.assertEqual(
                exc.args[0],
                "[eventlistener:dog] section sets " "invalid buffer_size (0)",
            )
    ...
    def test_event_listener_pool_disallows_redirect_stderr(self):
        ...
        try:
            ...
            self.assertEqual(
                exc.args[0],
                "[eventlistener:dog] section sets "
    ...
    def test_fcgi_programs_from_parser(self):
        ...
        self.assertEqual(gconf_foo.socket_config.url, "tcp://localhost:5000")
        ...
        self.assertEqual(gconf_bar.socket_config.url, "tcp://localhost:5001")

        gconf_cub = gconfigs[2]
        self.assertEqual(gconf_cub.name, "cub")
        self.assertEqual(gconf_cub.socket_config.url, "tcp://localhost:5002")
    ...
    def test_fcgi_programs_from_parser_with_environment_expansions(self):
        ...
        self.assertEqual(gconf_foo.name, "foo")
        self.assertEqual(gconf_foo.priority, 1)
        self.assertEqual(gconf_foo.socket_config.url, "tcp://localhost:9210")
        self.assertEqual(len(gconf_foo.process_configs), 2)
        pconfig_foo = gconf_foo.process_configs[0]
        self.assertEqual(pconfig_foo.__class__, FastCGIProcessConfig)
    ...
    def test_rpcinterfaces_from_parser(self):
        ...
        config.read_string(text)
        instance = self._makeOne()
        factories = instance.get_plugins(
            config, "supervisor.rpcinterface_factory", "rpcinterface:"
        )
    ...
    def test_clear_autochildlogdir_unlink_oserror(self):
        ...
        instance.logger = DummyLogger()
        instance.clear_autochildlogdir()
        self.assertEqual(instance.logger.data, ["Failed to clean up '%r'" % filename])
    ...
    def test_openhttpservers_reports_socket_error_with_errno(self):
        ...
        self.assertEqual(len(recorder), 1)
        expected = (
            "Cannot open an HTTP server: socket.error "
    ...
    def test_drop_privileges_user_none(self):
        instance = self._makeOne()
        self.assertRaises(NotImplementedError, instance.drop_privileges, None)
    ...
    def test_make_dispatchers_stderr_not_redirected(self):
        ...
        with TempFileOpen() as stdout_logfile:
            with TempFileOpen() as stderr_logfile:
                ...
                self.assertEqual(
                    dispatchers[stdout].event_type, ProcessCommunicationStdoutEvent
                )
                self.assertEqual(dispatchers[stderr].channel, "stderr")
                from supervisor.events import ProcessCommunicationStderrEvent

                self.assertEqual(
                    dispatchers[stderr].event_type, ProcessCommunicationStderrEvent
                )
    ...
    def test_make_dispatchers(self):
        ...
        with TempFileOpen() as stdout_logfile:
            with TempFileOpen() as stderr_logfile:
                ...
                self.assertEqual(
                    dispatchers[sys.stdout].process.listener_state,
                ...
                self.assertEqual(
                    dispatchers[sys.stderr].event_type, ProcessCommunicationStderrEvent
                )
    ...
    def test_make_dispatchers(self):
        ...
        with TempFileOpen() as stdout_logfile:
            with TempFileOpen() as stderr_logfile:
                ...
                dispatchers, pipes = instance.make_dispatchers(process1)
                self.assertEqual(dispatchers[sys.stdin].channel, "stdin")
                self.assertEqual(dispatchers[sys.stdin].closed, True)
                self.assertEqual(dispatchers[sys.stdout].channel, "stdout")
                from supervisor.events import ProcessCommunicationStdoutEvent

                self.assertEqual(
                    dispatchers[sys.stdout].event_type, ProcessCommunicationStdoutEvent
                )
                self.assertEqual(pipes["stdout"], sys.stdout)
                self.assertEqual(dispatchers[sys.stderr].channel, "stderr")
                from supervisor.events import ProcessCommunicationStderrEvent
...
class UnhosedConfigParserTests(unittest.TestCase):
    ...
    def test_saneget_no_default(self):
        ...
        self.assertRaises(
            ConfigParser.NoOptionError, parser.saneget, "supervisord", "missing"
        )
    ...
    def test_saneget_with_default_no_expand(self):
        ...
        parser.expansions = {"pet": "dog"}
        parser.read_string("[supervisord]\n")
        result = parser.saneget(
            "supervisord", "foo", default="%(pet)s", do_expand=False
        )
