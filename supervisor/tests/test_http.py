...
class TailFProducerTests(unittest.TestCase):
    ...
    @unittest.skipIf(
        sys.platform.startswith("win"), "Very different behaviour on Windows"
    )
...
class DeferringHttpRequestTests(unittest.TestCase):
    ...
    def test_done_http_11_connection_close(self):
        channel = self._makeChannel()
        inst = self._makeOne(
            channel=channel, version="1.1", header=["Connection: close"]
        )
        inst.done()
        self.assertTrue(channel.closed)
...
class DeferringHttpChannelTests(unittest.TestCase):
    ...
    def _makeOne(self):
        return self._getTargetClass()(server=None, conn=None, addr=None)
