...
class TestEventTypes(unittest.TestCase):
    ...
    def test_ProcessCommunicationStdoutEvent_inheritence(self):
        ...
        self.assertTrue(
            issubclass(ProcessCommunicationStdoutEvent, ProcessCommunicationEvent)
        )
    ...
    def test_ProcessCommunicationStderrEvent_inheritence(self):
        ...
        self.assertTrue(
            issubclass(ProcessCommunicationStderrEvent, ProcessCommunicationEvent)
        )
...
class TestSerializations(unittest.TestCase):
        ...
        if headerdata:
            try:
                ...
                raise AssertionError(
                    "headerdata %r could not be deserialized" % headerdata
                )
    ...
    def test_process_state_exited_event_expected(self):
        ...
        process1.group = DummyGroup
        process1.pid = 1
        event = events.ProcessStateExitedEvent(
            process1, ProcessStates.STARTING, expected=True
        )
        headers, payload = self._deserialize(event.payload())
    ...
    def test_process_state_exited_event_unexpected(self):
        ...
        process1.pid = 1
        event = events.ProcessStateExitedEvent(
            process1, ProcessStates.STARTING, expected=False
        )
        headers, payload = self._deserialize(event.payload())
        self.assertEqual(len(headers), 5)
        self.assertEqual(headers["processname"], "process1")
