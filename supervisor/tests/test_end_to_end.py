...
class EndToEndTests(BaseTestCase):
    ...
    def test_issue_638(self):
        ...
        if is_py2:
            ...
            supervisord.expect_exact(
                "gave up: produce-unicode-error entered FATAL state, "
    ...
    def test_issue_1170a(self):
        ...
        filename = pkg_resources.resource_filename(
            __name__, "fixtures/issue-1170a.conf"
        )
        args = ["-m", "supervisor.supervisord", "-c", filename]
        supervisord = pexpect.spawn(sys.executable, args, encoding="utf-8")
        self.addCleanup(supervisord.kill, signal.SIGINT)
    ...
    def test_issue_1170b(self):
        ...
        filename = pkg_resources.resource_filename(
            __name__, "fixtures/issue-1170b.conf"
        )
        args = ["-m", "supervisor.supervisord", "-c", filename]
        supervisord = pexpect.spawn(sys.executable, args, encoding="utf-8")
        self.addCleanup(supervisord.kill, signal.SIGINT)
    ...
    def test_issue_1170c(self):
        ...
        filename = pkg_resources.resource_filename(
            __name__, "fixtures/issue-1170c.conf"
        )
        args = ["-m", "supervisor.supervisord", "-c", filename]
        supervisord = pexpect.spawn(sys.executable, args, encoding="utf-8")
        self.addCleanup(supervisord.kill, signal.SIGINT)
    ...
    def test_issue_1231a(self):
        ...
        filename = pkg_resources.resource_filename(
            __name__, "fixtures/issue-1231a.conf"
        )
        args = ["-m", "supervisor.supervisord", "-c", filename]
        supervisord = pexpect.spawn(sys.executable, args, encoding="utf-8")
        self.addCleanup(supervisord.kill, signal.SIGINT)
    ...
    def test_issue_1231b(self):
        ...
        unicode, it should not fail."""
        filename = pkg_resources.resource_filename(
            __name__, "fixtures/issue-1231b.conf"
        )
        args = ["-m", "supervisor.supervisord", "-c", filename]
        supervisord = pexpect.spawn(sys.executable, args, encoding="utf-8")
        self.addCleanup(supervisord.kill, signal.SIGINT)
        ...
        for i in range(1, 4):
            ...
            try:
                ...
                self.assertIn(
                    "Unable to write Unicode to stdout because it " "has encoding ",
    ...
    def test_issue_1483a_identifier_default(self):
        ...
        filename = pkg_resources.resource_filename(
            __name__, "fixtures/issue-1483a.conf"
        )
        args = ["-m", "supervisor.supervisord", "-c", filename]
        supervisord = pexpect.spawn(sys.executable, args, encoding="utf-8")
        self.addCleanup(supervisord.kill, signal.SIGINT)
    ...
    def test_issue_1483c_identifier_from_command_line(self):
        ...
        filename = pkg_resources.resource_filename(
            __name__, "fixtures/issue-1483c.conf"
        )
        args = [
            "-m",
